...
const garminActivities = [
  { date: "Today · 12:04", name: "Treadmill Running", distance: "7.40 km", duration: "1h 00m 50s", avgHr: "108 bpm", maxHr: "143 bpm", detail: "6,978 steps" },
  { date: "17 Sep · 08:55", name: "Richmond Other", distance: "37.32 km", duration: "1h 08m 37s", avgHr: "129 bpm", maxHr: "152 bpm", detail: "+314 m elevation" },
  { date: "16 Sep · 10:54", name: "County Down Other", distance: "11.94 km", duration: "1h 06m 18s", avgHr: "137 bpm", maxHr: "159 bpm", detail: "+149 m elevation" },
  { date: "13 Sep · 08:25", name: "County Down Other", distance: "73.11 km", duration: "3h 12m 08s", avgHr: "107 bpm", maxHr: "169 bpm", detail: "+517 m elevation" },
  { date: "12 Sep · 15:47", name: "County Down Other", distance: "10.63 km", duration: "59m 48s", avgHr: "131 bpm", maxHr: "152 bpm", detail: "+114 m elevation" }
];
...
export default function Home() {
  ...
  return <main className="min-h-screen bg-[#071b1b] text-[#edf7f4]">
    <div className="mx-auto max-w-7xl px-5 py-6 lg:px-10">
      ...
      <section className="mt-7 rounded-2xl border border-cyan-300/25 bg-[#0d2929] p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm text-cyan-300">GARMIN CONNECT · LAST SYNC 18 SEP</p><h2 className="mt-1 text-xl font-semibold">Recent activity & heart-rate data</h2></div><span className="rounded-full bg-cyan-300/10 px-3 py-1 text-sm text-cyan-100">5 activities found</span></div><div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-5">{garminActivities.map(activity => <article className="rounded-xl bg-white/5 p-4" key={`${activity.date}-${activity.name}`}><p className="text-xs text-white/45">{activity.date}</p><p className="mt-2 font-medium">{activity.name}</p><p className="mt-3 text-lg font-semibold text-cyan-100">{activity.distance}</p><p className="text-sm text-white/55">{activity.duration}</p><dl className="mt-4 space-y-1 border-t border-white/10 pt-3 text-sm"><div className="flex justify-between gap-2"><dt className="text-white/45">Avg HR</dt><dd>{activity.avgHr}</dd></div><div className="flex justify-between gap-2"><dt className="text-white/45">Max HR</dt><dd>{activity.maxHr}</dd></div><div className="flex justify-between gap-2"><dt className="text-white/45">Extra</dt><dd className="text-right">{activity.detail}</dd></div></dl></article>)}</div><p className="mt-5 text-sm text-white/45">HRV, sleep, resting heart rate and stress were not supplied by Garmin for today, so they are shown as unavailable rather than estimated.</p></section>
